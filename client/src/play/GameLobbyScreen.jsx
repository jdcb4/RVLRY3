...
const HATGAME_MAX_CLUE_LENGTH = 80;
...
const getStartWarning = ({ roomState, game, isHost, allPlayersReady }) => {
  ...
  if (game.gameplayView === 'whowhatwhere' || game.gameplayView === 'hatgame') {
    ...
    if (game.gameplayView === 'hatgame') {
      ...
      if (
        roomState.players.some((player) => (clueCountsByPlayerId[player.id] ?? 0) !== requiredClues)
      ) {
...
export function GameLobbyScreen() {
  ...
  const handleSubmitHatClues = async (clues) => {
    return submitHatClues(roomCode, clues);
  };
